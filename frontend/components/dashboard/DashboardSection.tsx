...
import type { Deployment, DeploymentAction, FileEntry, InstanceStats, Stats } from "@/lib/types";
    ...
    <section className="space-y-6">
            ...
            <tbody className="divide-y divide-zinc-100">
              {deployments.map((deployment) => (
                <tr
                  ...
                  className={`cursor-pointer transition hover:bg-zinc-50/80 ${selectedDeployment?.id === deployment.id ? "bg-cyan-50/50" : ""}`}
                >
      ...
      {selectedDeployment ? (
        <div className="space-y-6">
            ...
            <div className="lg:col-span-2 rounded-xl border !border-orange-500 bg-white p-5 shadow-sm space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-2 border-t border-zinc-100 pt-4">
                <button
                  ...
                  className="h-8 rounded-md px-3 text-xs font-semibold  hover:bg-zinc-800 transition"
                >
                  Sync Diagnostics
                </button>
                <button
                  onClick={() => fetchLogs().catch((error) => alert(getErrorMessage(error)))}
                  className="h-8 rounded-md border border-zinc-200 bg-white px-3 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 transition"
                >
                  Pull Stdout Buffer
                </button>
                <button 
                  onClick={toggleStream} 
                  className="h-8 rounded-md border border-cyan-200 bg-cyan-50 px-3 text-xs font-semibold text-cyan-700 hover:bg-cyan-100/70 transition"
                >
                  {isStreaming ? "Kill Live Interceptor" : "Stream Architecture Telemetry"}
                </button>
                <a
          ...
          <div className="grid gap-6 xl:grid-cols-2 border !border-orange-500 rounded-xl bg-white p-5 shadow-sm">
            <LogPanel title="Buffered Console Payload (REST)" label="Static Tail" content={logs || "Load container logs to print state framework maps."} />
            <LogPanel
              title="Asynchronous Active Wire Stream"
              label={isStreaming ? "Streaming Active" : "Socket Standby"}
              content={liveLogs.length ? liveLogs.join("\n") : "Establish active client socket listeners to stream line arrays live."}
              accent
            />
          </div>
          ...
          <div className="rounded-xl border !border-orange-500 bg-zinc-50/50 p-5 border-dashed">
            <div>
              <h3 className="text-sm font-bold text-zinc-800">Asymmetric Asset File Ingestion Injection Pipeline</h3>
